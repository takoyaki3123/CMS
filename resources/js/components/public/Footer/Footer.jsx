import React from "react";
import PropTypes from 'prop-types';
const Footer = (props) => {

  const gade = (url, data) => {
    history.push(url,data);
  }
  return (
    <div className={`container ${props.isLogin?"d-block":"d-none"}`}>
      <footer className="d-flex flex-wrap justify-content-between align-items-center py-3 mt-4 border-top">
        <p className="col-md-4 mb-0 text-body-secondary">© 2024 Company, Inc</p>

        <a className="col-md-4 d-flex align-items-center justify-content-center mb-3 mb-md-0 me-md-auto link-body-emphasis text-decoration-none">
          <svg className="bi me-2" width="40" height="32"><use xlink:href="#bootstrap"></use></svg>
        </a>

        <ul className="nav col-md-4 justify-content-end">
          <li className="nav-item"><a href="#" onClick={()=>gade("/cms/home",{})}  className="nav-link px-2 text-body-secondary">Home</a></li>
        </ul>
      </footer>
    </div>
  )
}
Footer.propTypes = {
  isLogin: PropTypes.bool,
}
export default Footer;
